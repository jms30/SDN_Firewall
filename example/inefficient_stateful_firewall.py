            ...
            if(out_port != ofproto.OFPP_FLOOD) and (ethtype == ETH_TYPE_IP):
                ...
                if(ipo.proto == IPPROTO_ICMP):
                    ...
                    if ((icmpob.type==ICMP_PING) and self.inner_policy.has_key(ipo.src)):
                        ...
                        for i in range(0,len(self.icmp_conn_track.get(ipo.src))): 
                            if self.icmp_conn_track.get(ipo.src)[i][0] == ipo.dst:
                                xyz = self.icmp_conn_track.get(ipo.src)[i]
                ...
                #Check for TCP.    
                    ...
                    # TCP SYN ACK packet                   
                        ...
                        if self.tcp_conn_track.has_key(ipo.src):
                            for i in range(0,len(self.tcp_conn_track.get(ipo.src))):
                                if self.tcp_conn_track.get(ipo.src)[i][0] == ipo.dst:
                                    xyz = self.tcp_conn_track.get(ipo.src)[i]
