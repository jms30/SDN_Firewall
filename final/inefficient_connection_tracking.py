...
class InefficientFirewall(app_manager.RyuApp):
    ...
    """
    def conn_track_dict(self,s_ip,d_ip,s_port,d_port,act):
        self.logger.info("in conn_track_dict")
        self.s_ip = str(s_ip)
        self.d_ip= d_ip
        self.s_port = s_port
        self.d_port = d_port
        ...
        list1 = [self.d_ip,self.s_port,self.d_port,self.act]
        listobj = []
        # For new flow initiators.
        if(self.conn_track_dic.has_key(self.s_ip) is False):
            key = self.s_ip
            tup = tuple(list1)
            listobj.append(tup)
            tup = tuple(listobj)
            self.conn_track_dic[key] = tup
                
        # For existing Source of the new flow.
        elif (self.conn_track_dic.has_key(self.s_ip) is True):
            key = self.s_ip
            dst = self.conn_track_dic[key]
            dst = list(dst)
            ...
            if(out_port != ofproto.OFPP_FLOOD) and (ethtype == ETH_TYPE_IP):
                ...
                #Check for TCP.    
                            ...
                            for i in range(0,len(self.inner_policy.get(ipo.src))):
                                if self.inner_policy.get(ipo.src)[i][0] == ipo.dst:
                                    ...
                                    if((xyz[1]=='TCP') and (int(xyz[2]) == tcpo.src_port) and (int(xyz[3]) == tcpo.dst_port) and (xyz[4] == 'NEW')  and  (xyz[5] == 'ALLOW')):
                                                actions_default = action_fwd_to_out_port
                                                self.conn_track_dict(ipo.src, ipo.dst, tcpo.src_port, tcpo.dst_port, xyz[5])
                                                self.logger.info("conn_track_dic modified")
                                                break
                                    else:
                                        actions_default = action_drop
                                else:
                        ...
                        if self.inner_policy.has_key(ipo.src):
                            for i in range(0,len(self.inner_policy.get(ipo.src))):
                                if self.inner_policy.get(ipo.src)[i][0] == ipo.dst:
                                    xyz = self.inner_policy.get(ipo.src)[i]
                                    if(xyz[1]=='TCP'):
                                        if ((int(xyz[2]) == tcpo.src_port) and (int(xyz[3]) == tcpo.dst_port)):
                                            if ((xyz[4] == 'EST')  and  (xyz[5] == 'ALLOW')):
                                                actions_default = action_fwd_to_out_port
                                                self.conn_track_dict(ipo.src, ipo.dst, tcpo.src_port, tcpo.dst_port, xyz[5])
                                                self.logger.info("conn_track_dic modified")
                                                break
                    ...
                    # All remaining TCP packets, like, ACK, PUSH, FIN etc.            
                        ...
                        if self.conn_track_dic.has_key(ipo.src):
                            for i in range(0,len(self.conn_track_dic.get(ipo.src))):
                                if self.conn_track_dic.get(ipo.src)[i][0] == ipo.dst: 
                                    xyz = self.conn_track_dic.get(ipo.src)[i]
                                    if ((int(xyz[1]) == tcpo.src_port) and (int(xyz[2]) == tcpo.dst_port)):
                                        if (xyz[3] == 'ALLOW'):
                                            actions_default = action_fwd_to_out_port
                                            break
                                        else:
                            ...
                            self.logger.info("The packet was dropped coz we dont have seen handshake.")
                            actions_default = action_drop
